...
"
" Buffer Variables:
"     b:systemverilog_indent_modules : indenting after the declaration
"				 of module blocks
"     b:systemverilog_indent_width   : indenting width
"     b:systemverilog_indent_verbose : verbose to each indenting
"

" Only load this indent file when no other was loaded.
if exists("b:didsv_indent")
  finish
endif

let b:didsv_indent = 1
let b:did_indent = 1
...
function GetSystemVerilogIndent()
  ...
  elseif last_line =~ '^\s*\<\(function\|task\|class\)\>'
    if last_line !~ '\<end\>\s*' . vlog_comment . '*$' ||
      \ last_line =~ '\(//\|/\*\).*\(;\|\<end\>\)\s*' . vlog_comment . '*$'
      let ind = ind + offset
      if vverb
  ...
  elseif last_line =~ '\<\(virtual\|static\)\>\s*\<\(function\|task\|class\)\>'
    if last_line !~ '\<end\>\s*' . vlog_comment . '*$' ||
      \ last_line =~ '\(//\|/\*\).*\(;\|\<end\>\)\s*' . vlog_comment . '*$'
      let ind = ind + offset
      if vverb
  ...
  " `ifdef and `else
  elseif last_line =~ '^\s*`\<\(ifdef\|ifndef\|else\|elsif\)\>'
    let ind = ind + offset
    if vverb
